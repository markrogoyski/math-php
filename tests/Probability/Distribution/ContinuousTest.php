...
class ContinuousTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @dataProvider dataProviderForBetaPDF
     */
    public function testBetaPDF($α, $β, $x, $pdf)
    {
        $this->assertEquals($pdf, Continuous::betaPDF($α, $β, $x), '', 0.001);
    }

    public function dataProviderForBetaPDF()
    {
        return [
            [1, 1, 0, 1],
            [1, 1, 0.01, 1],
    ...
    public function testBetaPDFExceptionAlphaBetaLessThanEqualZero()
    {
        $this->setExpectedException('\Exception');
        list($α, $β, $x) = [0, -3, 4];
        Continuous::betaPDF($α, $β, $x);
    }

    public function testBetaPDFExceptionXOutOfBounds()
    {
        $this->setExpectedException('\Exception');
        list($α, $β, $x) = [1, 1, 4];
        Continuous::betaPDF($α, $β, $x);
