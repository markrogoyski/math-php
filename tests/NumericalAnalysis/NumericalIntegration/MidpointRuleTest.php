...
class MidpointRuleTest extends \PHPUnit\Framework\TestCase
{
    public function testSolvePolynomial()
    {
        ...
        // Error = 2 * ((3 - 0)²) = 18
        $tol = 18;
        $x = MidpointRule::approximate([[0, 1], [3, -2]]);
        ...
        // Error = 2 * ((1 - 0)² + (3 - 1)²) = 10
        $tol = 10;
        $x = MidpointRule::approximate([[0, 1], [1, 2], [3, -2]]);
        $this->assertEquals($expected, $x, '', $tol);
        ...
        $tol = 6;
        $x = MidpointRule::approximate([[0, 1], [1, 2], [2, 1], [3, -2]]);
        $this->assertEquals($expected, $x, '', $tol);

        // Same test as above but with points not sorted to test sorting works
        $tol = 6;
        $x = MidpointRule::approximate([[1, 2], [3, -2], [0, 1], [2, 1]]);
        $this->assertEquals($expected, $x, '', $tol);
