...
class Outlier
{
    ...
    public static function TietjenMooreStatistic(array $data, int $k, string $tails = "lower"): float
    {
        ...
        if ($tails == "two") {
            ...
            $kthlargest = Average::kthsmallest($z, $n - $k - 1);
            ...
            $kthlargest = Average::kthsmallest($data, $n - $k - 1);
            foreach ($data as $value) {
                if ($value <= $kthlargest) {
                    $smaller_set[] = $value;
                }
            ...
            $kthlargest = Average::kthsmallest($data, $k);
            foreach ($data as $value) {
                if ($value >= $kthlargest) {
                    $smaller_set[] = $value;
                }
     ...
     * @return float
     */
    public static function CriticalTietjenMoore(float $𝛼, int $n, int $k, string $tails = "lower"): float
    {
        ...
        for ($i = 0; $i < $number_of_simulations; $i++) {
            ...
            $CriticalList[] = self::TietjenMooreStatistic($data, $k, $tails);
        }
        return Average::kthsmallest($CriticalList, $𝛼 * $number_of_simulations - 1);
    }
}
